...
namespace YNL.Tools.UI
{
    ...
    public class TabAction : MonoBehaviour, ITabActionable
    {
        ...
        [SerializeField] private EButtonTransition _tabTransition = EButtonTransition.ColorAndTransform;

        [Title("Color")]
        [ShowIf("_tabTransition", Value = EButtonTransition.ColorAndTransform), SerializeField]
        private List<ImageTransitionColor> _imageTransition = new();
        [ShowIf("_tabTransition", Value = EButtonTransition.ColorAndTransform), SerializeField]
        private List<TMPTransitionColor> _tmpTransition = new();

        [Title("Transform")]
        [ShowIf("_tabTransition", Value = EButtonTransition.ColorAndTransform)]
        [SerializeField] private List<RectTransitionTransform> _transformTransition = new();

        private Animator _animator;
        [Title("Animation")]
        [ShowIf("_tabTransition", Value = EButtonTransition.Animation), SerializeField] private string _selectAnimation = "Select";
        [ShowIf("_tabTransition", Value = EButtonTransition.Animation), SerializeField] private string _deselectAnimation = "Deselect";
        #endregion
        ...
        {
            if (_tabTransition == Extension.Constant.EButtonTransition.Animation)
            {
        ...
        public void Select()
        {
            ...
            if (_tabTransition == Extension.Constant.EButtonTransition.ColorAndTransform)
            {
            ...
            else if (_tabTransition == Extension.Constant.EButtonTransition.Animation)
            {
